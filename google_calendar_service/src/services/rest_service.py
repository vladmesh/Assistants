...
class RestService:
    ...
    async def get_calendar_token(self, user_id: int) -> Optional[dict]:
        ...
        try:
            response = await self.client.get(f"{self.base_url}/api/calendar/user/{user_id}/token")
            if response.status_code == 404:
                return None
            response.raise_for_status()
            return response.json()
    ...
    async def update_calendar_token(self, user_id: int, access_token: str, refresh_token: str, token_expiry: datetime) -> bool:
        ...
        try:
            ...
            logger.error("Failed to update calendar token", error=str(e), user_id=user_id)
            return False
