...
async def add_tool_to_assistant(
    ...
    if existing_link:
        raise HTTPException(status_code=400, detail="Tool is already linked to this assistant")
    
    # Создаем новую связь
    link = AssistantToolLink(
...
async def remove_tool_from_assistant(
    ...
    if not link:
        raise HTTPException(status_code=404, detail="Tool is not linked to this assistant")
