...
async def create_message_endpoint(
    ...
    logger.info("Creating new message", role=message_in.role, user_id=message_in.user_id)
...
async def list_messages_endpoint(
    ...
    id_gt: Optional[int] = Query(None, description="Filter for messages with ID greater than this value"),
    id_lt: Optional[int] = Query(None, description="Filter for messages with ID less than this value"),
    role: Optional[str] = Query(None),
    status_filter: Optional[str] = Query(None, alias="status"), # Renamed to avoid conflict with status_code
    ...
    logger.info("Listing messages", user_id=user_id, assistant_id=assistant_id, limit=limit, offset=offset)
...
async def update_message_endpoint(
    ...
    if not db_message:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Message not found")
    updated_message = await message_crud.update(db=session, db_obj=db_message, obj_in=message_in)
