...
def upgrade() -> None:
    ...
    op.add_column(
        "user_summaries", sa.Column("assistant_id", sa.Uuid(), nullable=False)
    )
    op.add_column(
        "user_summaries",
        sa.Column("last_message_id_covered", sa.Integer(), nullable=True),
    )
    op.add_column(
        "user_summaries", sa.Column("token_count", sa.Integer(), nullable=True)
    )
    ...
    op.execute(
        "ALTER TABLE user_summaries ALTER COLUMN id TYPE INTEGER USING nextval('user_summaries_id_seq')"
    )
    op.execute(
        "ALTER TABLE user_summaries ALTER COLUMN id SET DEFAULT nextval('user_summaries_id_seq')"
    )

    op.drop_index("ix_user_summaries_secretary_id", table_name="user_summaries")
    op.create_index(
        op.f("ix_user_summaries_assistant_id"),
        "user_summaries",
        ["assistant_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_user_summaries_last_message_id_covered"),
        "user_summaries",
        ["last_message_id_covered"],
        unique=False,
    )
    op.drop_constraint(
        "user_summaries_secretary_id_fkey", "user_summaries", type_="foreignkey"
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "user_summaries",
        sa.Column("secretary_id", sa.UUID(), autoincrement=False, nullable=False),
    )
    op.drop_constraint(None, "user_summaries", type_="foreignkey")
    op.create_foreign_key(
        "user_summaries_secretary_id_fkey",
        "user_summaries",
        "assistant",
        ["secretary_id"],
        ["id"],
    ...
    op.drop_index(
        op.f("ix_user_summaries_last_message_id_covered"), table_name="user_summaries"
    )
    op.drop_index(op.f("ix_user_summaries_assistant_id"), table_name="user_summaries")
    op.create_index(
        "ix_user_summaries_secretary_id",
        "user_summaries",
        ["secretary_id"],
        unique=False,
    )
    op.execute(
        "ALTER TABLE user_summaries ALTER COLUMN id TYPE UUID USING gen_random_uuid()"
    )
