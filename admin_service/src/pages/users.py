...
def show_users_page(rest_client: RestServiceClient):
    ...
    st.subheader("Назначить секретаря")
    ...
    if selected_user:
        ...
        selected_secretary = st.selectbox(
            ...
            format_func=lambda x: "Без секретаря" if x is None else x.name,
        )
        ...
        if st.button("Назначить секретаря"):
            with st.spinner("Назначаем секретаря..."):
                if selected_secretary:
                    ...
                    run_async(rest_client.set_user_secretary(selected_user.id, None))
                    st.success(
