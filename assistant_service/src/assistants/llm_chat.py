...
class BaseLLMChat(BaseAssistant, ABC):
    ...
    async def process_message(self, message: BaseMessage, user_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            if isinstance(last_message, AIMessage):
                ...
                return last_message.get("content", "Извините, произошла ошибка при обработке сообщения")
            else:
                return str(last_message)
            
        except Exception as e:
            logger.error("Unexpected error in process_message",
                        error=str(e),
                        exc_info=True)
    ...
    async def close(self):
        ...
        try:
            ...
            logger.error("Error while closing assistant",
                        error=str(e),
                        exc_info=True) 