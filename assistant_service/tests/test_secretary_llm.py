...
class TestSecretaryLLM:
    ...
    async def test_basic_conversation(self):
        ...
        assert last_message
        # Ассистент может представиться по-разному, проверяем что он
        # вообще как-то отвечает на вопрос об имени
        assert any(
    ...
    async def test_assistant_with_subassistant(self):
        ...
        secretary = BaseLLMChat(
            ...
            Используй инструмент ask_expert ТОЛЬКО для
        ...
        # суб-ассистента
        assert "Технический ответ:" in responses[0]
