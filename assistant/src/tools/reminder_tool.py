...
class ReminderSchema(BaseModel):
    ...
    delay_seconds: Optional[int] = Field(None, description="Через сколько секунд отправить напоминание (альтернатива datetime)")
    datetime_str: Optional[str] = Field(None, description="Дата и время напоминания в формате ISO (YYYY-MM-DD HH:MM)")
    timezone: Optional[str] = Field(None, description="Временная зона для datetime_str (например, 'Europe/Moscow')")
    ...
    def validate_reminder_time(self) -> datetime:
        ...
        if self.delay_seconds is not None and self.datetime_str is not None:
            raise ValueError("Укажите либо delay_seconds, либо datetime_str с timezone, но не оба")
        ...
        if self.datetime_str is not None:
            if not self.timezone:
                raise ValueError("При указании datetime_str необходимо указать timezone")
            try:
        ...
        raise ValueError("Необходимо указать либо delay_seconds, либо datetime_str с timezone")
...
class ReminderTool(BaseTool):
    ...
    def _run(self, message: str, delay_seconds: Optional[int] = None, 
             datetime_str: Optional[str] = None, timezone: Optional[str] = None) -> str:
        """Синхронный метод не используется"""
        raise NotImplementedError("Этот инструмент поддерживает только асинхронные вызовы")
    
    async def _execute(self, message: str, delay_seconds: Optional[int] = None,
                      datetime_str: Optional[str] = None, timezone: Optional[str] = None) -> str:
        ...
        try:
            ...
            try:
                ...
                if delay_seconds is not None:
                    return f"Напоминание создано: {message} (через {delay_seconds} секунд)"
                else:
