...
class TestMultiSecretary:
    ...
    async def test_get_user_secretary(self, factory, mock_rest_client):
        ...
        mock_rest_client.get_user_secretary.assert_called_once_with(user_id)  # Проверяем, что API вызван только один раз

    @pytest.mark.asyncio
    async def test_different_users_get_different_secretaries(self, factory, mock_rest_client):
        """
        Тест проверяет:
        ...
        mock_rest_client.get_user_secretary.side_effect = [
            {"id": "secretary1", "name": "secretary1", "type": "llm", "instructions": "Ты - первый секретарь"},
            {"id": "secretary2", "name": "secretary2", "type": "llm", "instructions": "Ты - второй секретарь"}
        ]
        ...
        assert mock_rest_client.get_user_secretary.call_count == 2  # Проверяем, что API вызван дважды
    ...
    async def test_secretary_processing_messages(self, factory, mock_rest_client):
        ...
        assert "test_secretary" in last_message.lower()  # Проверяем, что секретарь представился 